...
public class GUI {
    ...
    private final JTextField[][] textField = new JTextField[9][9];
    private final GridPanel gridPanel = new GridPanel(new GridLayout(9, 9, 1, 1));
    ...
    public void GUIToSudokuTable() {
        for (int y = 0; y < 9; y++) {
            for (int x = 0; x < 9; x++) {
                if (!textField[x][y].getText().equals("")) {
                    /*
                     if a number is  in the textField,
                     put it in the sudoku table
                    */
    ...
    public void createGUI() {
        ...
        solveButton.addActionListener(e -> {
            if (!checkText())
                JOptionPane.showMessageDialog(frame, "Invalid input. Values must be integers from 1 to 9", "Error",
                        JOptionPane.ERROR_MESSAGE);
            else {
                GUIToSudokuTable();
                if (!sudokuTable.solve(1))
                    JOptionPane.showMessageDialog(frame, "This puzzle cannot be solved.", "Error",
                            JOptionPane.ERROR_MESSAGE);
                else
                    sudokuTableToGUI();
            }
